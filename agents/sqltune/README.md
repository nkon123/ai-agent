...
> 값을 **빠뜨렸을 때** 뿐 아니라 문장에 없는 바인드를 **더 넘겼을 때**도 난다.
>
> 처음에는 EXPLAIN PLAN 문에 대상 SQL 의 바인드가 그대로 들어가는데 값을
> 안 넘겨서 났다. `statement_id` 를 바인드로 쓰지 않도록 고쳤다.
>
> 그 뒤 LLM 이 만든 후보에서 다시 났다. 정규식으로 바인드를 짐작한 것이
> 원인이었다 — `:d1` 과 `:D1` 을 둘로 세거나(오라클은 같은 바인드다),
> `:1` 같은 위치 바인드를 놓치거나, 따옴표가 안 닫힌 SQL 에서 뒷부분을
> 통째로 문자열로 보고 건너뛰었다.
>
> 지금은 짐작하지 않는다. `cursor.prepare()` 뒤 `bindnames()` 로
> **드라이버에게 실제 이름을 묻고** 그 이름 그대로 값을 채운다.
> 사용자가 준 값의 대소문자가 달라도 맞춰 준다.

## 판정 기준은 문서 한 곳에 (11g 기준)
...
**코드가 내는 규칙에 문서 절이 없으면 테스트가 실패한다** — 그래야 한쪽만
늘어나지 않는다.

문서는 **Oracle 11g(11.2)** 를 대상으로 한다. 12c 이상에서만 되는 문법을
제안하면 사내에서 그대로 못 쓴다.

| 11g 에 없는 것 | 대신 |
|---|---|
| `FETCH FIRST n ROWS ONLY` | `ROWNUM` 페이징 (인라인뷰 2단) |
| 적응형 실행계획 | 통계가 전부다 |
| 하이브리드 히스토그램 | 도수분포 / 높이균형 |

| 11g 에 있는 것 | 측정할 때 주의 |
|---|---|
| 카디널리티 피드백(11.2) | **2회차부터 계획이 바뀔 수 있다** |
| 결과 캐시 | 2회차가 비현실적으로 빠르다 |
| 적응형 커서 공유 | 바인드 값을 바꿔 재면 계획이 달라진다 |

문서에는 규칙별로 **고친 예시**가 함께 있다. 함수 기반 인덱스·가상 컬럼,
`NOT IN` 의 NULL 함정, `ROWNUM` 페이징 형태, `DBMS_STATS` 수집 예시,
확장 통계, 인덱스 사용 감시 같은 11g 실무 항목을 담았다.

### 프롬프트에는 걸린 규칙의 '절 전체'만 넣는다

문서가 10KB 라 통째로 넣으면 컨텍스트를 넘긴다. 걸린 규칙의 절만 잘라
보낸다(예: 4개 규칙 → 4.4KB). 줄 단위로 뽑으면 규칙명이 있는 줄만 남고
정작 "어떻게 고치라"는 예시가 빠지므로 **절 단위**로 자른다.

11g 제약표와 8항(비교 기준)은 어느 규칙에 걸렸든 항상 붙인다 — 버전에
없는 문법을 내놓거나 건수가 달라지는 변경을 하면 안 되기 때문이다.
