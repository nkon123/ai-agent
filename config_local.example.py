...
# Ollama
# --------------------------------------------------------------------------
# 사고 과정(thinking). 대화는 켜고 에이전트 내부 호출은 끄는 것이 기본이다.
# 내부 호출은 형식이 정해져 있어 사고가 시간만 먹는다(수백 초 쓰고 본문이
# 비어 나오는 일이 잦다).
# 판정 정확도가 아쉬우면 AGENT_REASONING 을 켠다 — 대신 문장당 3초가 40초가 된다.
# CHAT_REASONING = True       # 비우면 모델 기본값
