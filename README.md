...
대화 목록은 **브라우저에만 저장된다**(localStorage, 최근 50개).
클릭하면 그 대화로 돌아간다.

서버의 대화 기억(체크포인터)은 프로세스 메모리라 **서버를 다시 띄우면
사라진다.** 그러면 화면에는 이전 대화가 보이는데 모델은 앞 내용을 기억하지
못한다. 그래서 서버 기동 시각을 화면이 받아 두고, 재시작 뒤 옛 대화를 열면
그 사실을 알려 준다.

좁은 화면에서는 옆칸이 접힌다 (1100px 이하 오른쪽, 820px 이하 왼쪽).
...
### 사고 과정(thinking)은 대화만 켠다

| 어디 | 기본 | 왜 |
|---|---|---|
| 대화(챗봇) | **켬** (`CHAT_REASONING`) | 어떤 툴을 부를지, 결과를 어떻게 전할지 판단해야 한다 |
| 에이전트 내부 | **끔** (`AGENT_REASONING`) | 구조화 출력이라 형식이 이미 정해져 있다. 사고에만 수백 초를 쓰고 본문이 비어 나오는 일이 잦다 |
| `sqltune` 후보 생성 | 항상 끔 | 코드에서 명시한다. 켜면 392·534초를 쓰고 실패했다 |

같은 질문이 **153초 → 30초**로 줄었다(문장 판정이 40초 → 2초씩).

다만 판정 정확도는 떨어질 수 있다. 3건짜리 표본에서 `impact` 판정이
사고를 켜면 3/3, 끄면 1/3 이었다. 규칙 판정은 그대로이고 LLM 재확인만
달라지므로 결과가 뒤집히지는 않지만, 정확도가 중요하면 켠다.

```bash
AGENT_REASONING=true python app/app.py     # 내부 호출도 사고를 켠다
```
